...
4. **Students are identified by system-generated IDs.** Each roster entry gets a random UUID. Teachers choose the display labels for their students; the system never requires real names.
...
| Roster entries: teacher-chosen labels, system-generated UUIDs, grade information | Plaintext teacher email addresses (only a SHA-256 hash) |
| A tamper-resistant audit log of actions taken | Plaintext IP addresses (only a SHA-256 hash in the audit log) |
...
- **Strict data isolation.** Every database query is scoped to the signed-in teacher's account. Before any record is read, updated, or deleted, the system verifies the record belongs to that teacher. One teacher can never see another teacher's students or analyses.
- **Encryption of sensitive content.** All AI-generated insight text is encrypted with AES-256-GCM (with a unique initialization vector per field) before storage, and the database connection itself uses TLS.
...
- **Tamper-resistant audit trail.** Every significant action — analyses, roster changes, saved insights, admin views — is recorded in an append-only audit log. Database permissions prevent the application itself from editing or deleting audit entries.
